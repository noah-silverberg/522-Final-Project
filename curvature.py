...
def get_loss_samples_filternorm(net, dataloader, criterion, steps=21, range_limit=1.0, cuda=True):
    ...
    d2 = net_plotter.create_random_direction(net, dir_type='weights', ignore='biasbn', norm='filter')
    directions = [d1, d2]

    # 2. Define the Grid (The "Slice")
    # We scan from -range to +range. 
    # We ensure 0 (the original model) is included by using an odd number of steps.
    alphas = np.linspace(-range_limit, range_limit, steps)
    betas = np.linspace(-range_limit, range_limit, steps)
    
    # Save original weights to restore later
    original_weights = net_plotter.get_weights(net)
